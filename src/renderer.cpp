...
vertexObject renderer::createVertexObject(float vertices[], unsigned int indices[], unsigned int vertCount, unsigned int triangles){
unsigned int VAO;
...
unsigned char *data = stbi_load((RESOURCES_PATH "textures/" + path).c_str(), &width, &height, &nrChannels, STBI_rgb_alpha);
